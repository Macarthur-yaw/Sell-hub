            ...
            </div>
{
    show && (
        <div className='bg-black absolute top-0'>
            ...
            {
                show && (
<div onClick={()=>setShow?.(prevValue=>!prevValue)} className='bg-white fixed inset-0 h-screen w-full z-20'>
             ...
             <section className={`${show ? 'px-2':'px-2'}`}>    
                <div onClick={()=>setShow?.(false)} className={`${show ? 'absolute z-20 top-0 right-0 p-2':'hidden'}`}>
                <CloseIcon />
                </div>
                
                   <div
             onClick={()=>setShow?.(true)}
             className={`${show ? 'flex flex-col  px-2 absolute z-30 w-[90%] left-1/2   transform -translate-x-1/2 ':'flex flex-row items-center px-2'} `}>
